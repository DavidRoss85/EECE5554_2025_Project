...
class PincherX100IK:
    ...
    def solve(self, T, elbow='up'):
        ...
        # Note: This is conservative - actual reach with joint rotation is higher
        # For now, use 1.5x safety factor to allow shoulder rotation
        max_reach_conservative = (Lr + self.L3) * 1.5
        if c > max_reach_conservative:
            raise ValueError(f"Position unreachable: distance {c:.4f}m exceeds max reach {max_reach_conservative:.4f}m (conservative: {Lr + self.L3:.4f}m)")
